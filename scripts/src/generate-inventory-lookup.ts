...
async function generateInventoryLookup(): Promise<void> {
	...
	const metadataOutputPath = path.join(
		__dirname,
		'../data/processed/inventory-lookup-metadata.json'
	);
	...
	return new Promise((resolve, reject) => {
		fs.createReadStream(inventoryPath)
			...
			.on('end', async () => {
				...
				console.log(
					`  - Lookup size: ${(lookupJson.length / 1024 / 1024).toFixed(1)}MB uncompressed`
				);
				console.log(
					`  - Lookup size: ${(compressed.length / 1024 / 1024).toFixed(1)}MB compressed`
				);
