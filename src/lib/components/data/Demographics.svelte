...
{:else}
	<AreaContext {data} />
	<table class="text-earth/80 text-2xs w-full border-collapse sm:text-xs font-sans">
		<tbody>
			<tr>
			...
			<tr>
				...
				<td class="py-0.5 text-right sm:py-1">{formatPercent(data.pct_poverty)}</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="py-0.5 text-right sm:py-1">{formatPercent(data.pct_minority)}</td>
			</tr>
		</tbody>
	</table>
{/if}
